...
class ArticlePrinter(object):
    ...
    def print_paragraph_stats(self):
        ...
        for item, para in enumerate(self.article.paragraphs):
            sentences = (
                Color("{red}%s{/red}" % str(len(para)))
                if len(para) > 5
                else str(len(para))
            )
            avg_words_per_sentence = (
                Color("{red}%s{/red}" % str(para.avg_words_per_sentence))
                if para.avg_words_per_sentence > 25
                else str(para.avg_words_per_sentence)
            )
            table_data.append(
                [
                    item + 1,
            ...
            table_data.append(
                [
                    "",
                    "Flesh Reading score={flesch_reading}, Dale Chall Readability= {dale_chall}".format(
        ...
        if len(words) >= 1:
            ...
            if display_count:
                msg = "{red} **- %s: %s {/red}\r\n" % (
                    heading,
                    ", ".join(
                        format_str.format(word=str(word), count=words.count(word))
                        for word in words_unique_list
                    ),
                )
            else:
                msg = "{red} **- %s: %s {/red}\r\n" % (
                    heading,
                    ", ".join(word for word in words_unique_list),
                )
            print(Color(msg))

    def print_detail(self):
        self._print_detail_of(
            self.article.get_compulsive_hedgers(),
            "Compulsive Hedgers",
            display_count=False,
        )
        self._print_detail_of(
            self.article.get_intensifiers(), "Intensifiers", display_count=False
        )
        self._print_detail_of(self.article.get_vague_words(), "Vague words")
        self._print_detail_of(
            self.article.ten_words_with_most_syllables(),
            "10 words with most syllables",
            display_count=False,
        )
        self._print_detail_of(
            self.article.get_n_most_repeated_words(20),
